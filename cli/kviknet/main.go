...
import (
	...
	"github.com/fasmide/hipermads/kviknet"
	"gopkg.in/gomail.v2"
)

// SentIDs represents invoice IDs which we already emailed
type SentIDs map[string]bool

func main() {

	// before communicating with kviknet, lets get our own data stright
	fd, err := os.Open(os.Getenv("KVIKNET_ID_DB_PATH"))
	if err != nil {
	...
	// communicate with kviknet - lookup first page of invoices
	session, err := kviknet.Login(os.Getenv("KVIKNET_USERNAME"), os.Getenv("KVIKNET_PASSWORD"))
	if err != nil {
	...
	if err != nil {
		log.Fatalf("unable to lookup invoices: %s", err)
	}
	...
	for _, invoice := range invoices {
		...
		// compared to hiper - we also dont want to send "Indbetaling" types
		if invoice.Type == "Indbetaling" {
			continue
		}

		log.Printf("Will send invoice %s of a total amount of %s", invoice.ID, invoice.Amount)

		m := gomail.NewMessage()
		m.SetHeader("From", os.Getenv("HIPER_EMAIL_FROM"))
		m.SetHeader("To", os.Getenv("HIPER_EMAIL_TO"))
		m.SetHeader("Subject", fmt.Sprintf("Regning %s fra kviknet på %s", invoice.ID, invoice.Amount))
		...
		m.Attach(fmt.Sprintf("%s.pdf", invoice.ID), gomail.SetCopyFunc(func(w io.Writer) error {
			// attach the invoice - downloaded directly from kviknet
...
func StoreIds(s SentIDs) {
	fd, err := os.Create(os.Getenv("KVIKNET_ID_DB_PATH"))
	if err != nil {
